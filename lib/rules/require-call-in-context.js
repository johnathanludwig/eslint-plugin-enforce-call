...
function getFirstStatementCalls(node) {
  ...
  /**
   * Collect a direct call from an expression, unwrapping await expressions.
   * @param {Node} expression - Expression to inspect
   * @param {string[]} target - Array receiving the call name
   */
  function collectCallFromExpression(expression, target) {
    ...
    const name = getCallExpressionName(current);
    if (name) {
      target.push(name);
    }
  }
